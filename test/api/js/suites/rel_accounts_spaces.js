module.exports = async function (c) {
  ...
  let result = await c.get('get account with relationships', `/accounts/${account.id}?relationshipLevels=1`)
  c.assertEqualKeys(['name', 'id'], result.data.spaces, [space, secondSpace])

  result = await c.get('list accounts with relationships as super user', `/accounts?relationshipLevels=1`, {headers: superHeaders})
