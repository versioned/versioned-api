...
module.exports = async function (c) {
  await c.get({it: 'GET email_link where email doesnt exist', status: 404}, `/email_link/foo/bar`)
  ...
  await c.get('GET email_link where email exists', `/email_link/${email.id}/${encodeURIComponent(url)}`)
  ...
  await c.get('GET email_link where email exists again', `/email_link/${email.id}/${encodeURIComponent(url)}`)
  email = await emails.get(email.id, {allowMissing: false})
