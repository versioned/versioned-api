...
module.exports = async function (c) {
  ...
  c.assertEqual(result.data.version, 2)
  c.assertEqual(result.data.publishedVersion, 1)
  c.assertEqual(result.data.sys.versions.length, 2)
  c.assertEqual(result.data.sys.versions[0].id, id)
  c.assertEqual(result.data.sys.versions[0].version, 2)
  c.assertEqual(result.data.sys.versions[0].title, 'foobar2')
  c.assertRecent(result.data.sys.versions[0].createdAt)
  c.assertEqual(result.data.sys.versions[0].createdBy.id, c.data.user.id)
  c.assert(!result.data.sys.versions[0].updatedAt)
  c.assert(!result.data.sys.versions[0].updatedBy)
  c.assertEqual(result.data.sys.versions[1].id, id)
  c.assertEqual(result.data.sys.versions[1].version, 1)
  c.assertEqual(result.data.sys.versions[1].title, 'foobar1')
  c.assertEqual(result.data.sys.versions[1].createdBy.id, c.data.user.id)
  c.assertEqual(result.data.sys.versions[1].updatedBy.id, c.data.user.id)
  c.assertRecent(result.data.sys.versions[1].createdAt)
  c.assertRecent(result.data.sys.versions[1].updatedAt)
  c.assert(new Date(result.data.sys.versions[1].updatedAt) >= new Date(result.data.sys.versions[1].createdAt))
  c.assert(new Date(result.data.sys.versions[1].createdAt) >= new Date(result.data.sys.versions[0].createdAt))
  ...
  result = await c.get('check versions', `/data/${spaceId}/items/${id}?versions=1`)
  c.assertEqual(result.data.sys.versions.length, 2)
  c.assertEqual(result.data.sys.versions[0].title, 'foobar2')
  c.assertEqual(result.data.sys.versions[0].version, 2)
  c.assertEqual(result.data.sys.versions[1].title, 'foobar1')
  ...
  result = await c.put('update again (update field)', `/data/${spaceId}/items/${id}`, {title: 'foobar3'})
  ...
  c.assertEqual(result.data.version, 2)
  c.assertEqual(result.data.sys.versions.length, 2)
  c.assertEqual(result.data.sys.versions[0].title, 'foobar3')
  c.assertEqual(result.data.sys.versions[0].version, 2)
  c.assertEqual(result.data.sys.versions[1].title, 'foobar1')
  c.assertEqual(result.data.sys.versions[1].version, 1)

  result = await c.put('update again (delete field)', `/data/${spaceId}/items/${id}`, {body: null})
  c.assert(!result.data.body)
  c.assertEqual(result.data.title, 'foobar3')
  c.assertEqual(result.data.version, 2)
  c.assertEqual(result.data.publishedVersion, 1)

  result = await c.get('check versions', `/data/${spaceId}/items/${id}?versions=1`)
  c.assert(!result.data.sys.versions[0].body)
  c.assertEqual(result.data.sys.versions[0].title, 'foobar3')
  c.assertEqual(result.data.sys.versions[0].version, 2)

  result = await c.put('update again (add field)', `/data/${spaceId}/items/${id}`, {body: item.body})
  c.assertEqual(result.data.body, item.body)
  c.assertEqual(result.data.title, 'foobar3')
  c.assertEqual(result.data.version, 2)
  c.assertEqual(result.data.publishedVersion, 1)

  result = await c.get('check versions', `/data/${spaceId}/items/${id}?versions=1`)
  c.assertEqual(result.data.sys.versions[0].body, item.body)
  c.assertEqual(result.data.sys.versions[0].title, 'foobar3')
  c.assertEqual(result.data.sys.versions[0].version, 2)
