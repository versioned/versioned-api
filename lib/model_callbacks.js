...
function modelCallbacks (logger = SILENT_LOGGER) {
  async function invokeCallbacks (api, doc, when, action, options = {}) {
    ...
    for (let callback of callbacks(model, when, action)) {
      if (shouldLog(action)) logger.verbose(`model_callbacks starting ${action} ${when} ${model.coll}.${getIn(doc, 'id')} ${callback.name} callback`)
      let returnValue = await callback(result, callbackOptions)
      if (shouldLog(action)) logger.verbose(`model_callbacks finished ${action} ${when} ${model.coll}.${getIn(doc, 'id')} ${callback.name} callback with return value diff:`, (returnValue && diff(result, returnValue)))
