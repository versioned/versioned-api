  ...
  async function invokeCallbacks (api, doc, when, action, options = {}) {
    ...
    for (let callback of callbacks(model, when, action)) {
      if (!skipCallbacks.includes(callback.name)) {
        ...
        if (shouldLog(action)) logger.verbose(`model_callbacks starting ${operation} ${when} ${callback.name} callback`)
        let returnValue = await callback(result, callbackOptions)
        if (shouldLog(action)) logger.verbose(`model_callbacks finished ${operation} ${when} ${callback.name} callback with return value diff:`, (returnValue && diff(result, returnValue)))
