  ...
  async function invokeCallbacks (api, doc, when, action, options = {}) {
    ...
    for (let callback of callbacks(model, when, action)) {
      if (!skipCallbacks.includes(callback.name)) {
        const operation = getIn(doc, 'id') ? `${model.coll}.${action}.${getIn(doc, 'id')}` : `${model.coll}.${action}`
        if (shouldLog(action)) logger.verbose(`model_callbacks starting ${callback.name} ${when} ${operation}`)
        try {
          let returnValue = await callback(result, callbackOptions)
          if (shouldLog(action)) logger.verbose(`model_callbacks finished ${callback.name} ${when} ${operation} with return value diff:`, (returnValue && diff(result, returnValue)))
          if (returnValue) result = returnValue
        } catch (error) {
          logger.error(`model_callbacks error ${callback.name} ${when} ${operation} doc=${json(doc)}`, error)
          throw error
        }
      }
    }
