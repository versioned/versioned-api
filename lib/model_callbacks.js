const {property, json, merge, getIn, flatten} = require('lib/util')
...
function callbacks (model, when, action) {
  ...
  const sortedCallbacksList = sortCallbacks(callbacksList)
  return sortedCallbacksList
}
...
function modelCallbacks (logger = SILENT_LOGGER) {
  async function invokeCallbacks (api, doc, when, action, options = {}) {
    ...
    const _callbacks = callbacks(model, when, action)
    if (shouldLog(action)) logger.verbose(`model_callbacks ${model.coll}.${action} ${when} callbacks order: ${_callbacks.map(property('name')).join(', ')}`)
