...
function modelCallbacks (logger = SILENT_LOGGER) {
  async function invokeCallbacks (model, doc, when, action, options = {}) {
    ...
    for (let callback of callbacks(model, when, action)) {
      if (shouldLog(action)) logger.verbose(`model_callbacks about to run ${model.coll} ${callback.name} callback`)
      let returnValue = await callback(result, callbackOptions)
      if (shouldLog(action)) logger.verbose(`model_callbacks finished ${model.coll} ${callback.name} callback with return value:`, returnValue)
