    ...
    for (let callback of callbacks(model, when, action)) {
      ...
      if (!skipCallbacks.includes(callback.name)) {
        ...
        try {
          ...
          if (shouldLog(action)) logger.verbose(`model_callbacks finished ${operation} elapsed=${elapsed(startTime)}, return value diff:`, (returnValue && diff(result, returnValue)))
          if (returnValue) result = returnValue
        } catch (error) {
          logger.error(`model_callbacks error ${operation} elapsed=${elapsed(startTime)} doc=${json(doc)}`, error)
          throw error
        }
      } else {
