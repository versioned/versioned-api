...
function length (container) {
  ...
  } else if (!nil(container) && typeof container === 'object') {
...
function truncated (value, limit = 50) {
  if (nil(value)) return ''
...
function getIn (obj, path, defaultValue) {
  ...
  for (const [index, key] of path.entries()) {
    if (isArray(result) && !validInt(key)) {
      return result.map(item => getIn(item, path.slice(index), defaultValue))
    }
...
function mergeIn (obj, path, value) {
  return updateIn(obj, path, (currentValue) => merge(currentValue, value))
}
...
function deepRename (obj, renames) {
  if (!obj) return undefined
  return keyValues(renames || {}).reduce((acc, [fromPath, toPath]) => {
    acc = setIn(acc, toPath, getIn(obj, fromPath))
    return setIn(acc, fromPath, undefined)
  }, obj)
}
...
function map (arrayOrValue, valueTransform) {
  if (isArray(arrayOrValue)) {
    return arrayOrValue.map(valueTransform)
  } else {
    return valueTransform(arrayOrValue)
  }
}
...
function validInt (value) {
  return (typeof value === 'number' && Number.isInteger(value)) ||
    (typeof value === 'string' && value.match(/^(?:[0-9]|[1-9][0-9]*)$/))
}
...
const json = (process.env.PRETTY_JSON === '1' ? prettyJson : unprettyJson)
