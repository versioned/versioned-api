const {merge, pick, intersection, isArray, keys, getIn, filter} = require('lib/util')
...
const updatable = (property) => getIn(property, ['x-meta', 'update']) !== false
...
function updatableProperties (model) {
  return keys(filter(modelMeta.properties(model), (p) => writable(p) && updatable(p)))
}

function readableProperties (model) {
  const readable = (property) => getIn(property, ['x-meta', 'readable']) !== false
  return keys(filter(modelMeta.properties(model), readable))
}
...
function updatableDoc (model, doc) {
  return doc && pick(doc, updatableProperties(model))
}
