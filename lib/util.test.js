...
test('getIn - takes an object and one or more property names (path) and returns the nested value in the object at given path or undefined', () => {
  ...
  expect(u.getIn({foo: {bar: [{baz: 3}, {baz: 5}]}}, ['foo', 'bar', 'baz'])).toEqual([3, 5])
...
test('deepRename - rename deep paths in object, overwrites any existing value at path', () => {
  expect(u.deepRename({foo: 1, fubba: 3, bar: 2}, {foo: 'fubba'})).toEqual({fubba: 1, bar: 2})
  expect(u.deepRename({title: {value: 'foobar'}, bar: 2}, {'title.value': 'title'})).toEqual({title: 'foobar', bar: 2})
  expect(u.deepRename({title: {foo: 'foobar'}, bar: 2}, {'title.foo': 'title.bar'})).toEqual({title: {bar: 'foobar'}, bar: 2})
})
