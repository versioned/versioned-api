...
test('safeCall - takes a function and an argument and invokes the function on the arg if it is safe (not nil by default) - otherwise returns the argument', () => {
  expect(u.safeCall(JSON.parse, null)).toEqual(null)
  expect(u.safeCall(JSON.parse, '{"foo": 1}')).toEqual({foo: 1})
})

test('safeCall - can take a third argument which is a function that determines if the function can safely be invoked', () => {
  const isJSON = (value) => {
    return typeof value === 'string' && value.startsWith('{')
  }
  expect(u.safeCall(JSON.parse, '', isJSON)).toEqual('')
  expect(u.safeCall(JSON.parse, '{"foo": 1}', isJSON)).toEqual({foo: 1})
})

test('safeCall - exceptions return undefined', () => {
  expect(u.safeCall(JSON.parse, new Date())).toEqual(undefined)
})
...
test('empty/notEmpty - checks for null/undefined and empty arrays, strings, and objects', () => {
  const emptyValues = [
    null,
    undefined,
    [],
  ...
  for (let value of emptyValues) {
    ...
    expect(u.notEmpty(value)).toBe(false)
  }
  ...
  for (let value of nonEmptyValues) {
    ...
    expect(u.notEmpty(value)).toBe(true)
  }
})
