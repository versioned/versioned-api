...
function modelController (model, getApi, responseModule, options = {}) {
  ...
  function apiOptions (req, options = {}) {
    const queryParams = options.queryParams || req.queryParams
    const scope = getScope(req)
  ...
  async function _list (req, queryParams) {
    const listOptions = [apiOptions(req, {queryParams}), queryParams, {count: true}].reduce(merge)
  ...
  async function _get (req, id, queryParams) {
    const query = {id}
    const getOptions = merge(apiOptions(req, {queryParams}), queryParams)
    const api = await getApi(req.space, model)
