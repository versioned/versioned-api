...
function externalModelApi (model, _mongo, logger = SILENT_LOGGER) {
  ...
  async function list (query = {}, options = {}) {
    ...
    if (ids) {
      ...
      docs = await Promise.all(ids.map(id => get(id, options)))
    } else {
      const route = getIn(options, 'queryParams.q') ? 'search' : 'list'
      const { dataPath, countPath, headers } = getIn(model, `routes.${route}`, {})
    ...
    docs = docs && docs.map(decorateDoc)
    if (options.count) {
  ...
  async function get (queryOrId, options = {}) {
    ...
    let {data} = await axios.request({url, headers})
    if (dataPath) data = getIn(data, dataPath)
    return decorateDoc(data)
  }
