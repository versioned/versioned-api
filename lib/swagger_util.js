...
function parameters (route) {
  ...
  if (route.requireAuth !== false) result = result.concat(authParam(route))
  return result
}
...
function parametersToSchema (parameters) {
  const properties = parameters.reduce((acc, parameter) => {
    if (parameter.schema && !getIn(parameter, 'x-meta.namePattern')) {
      acc[parameter.name] = parameter.schema
    }
    return acc
  }, {})
  const patternProperties = parameters.reduce((acc, parameter) => {
    const pattern = getIn(parameter, 'x-meta.namePattern')
    if (parameter.schema && pattern) {
      acc[pattern.toString()] = parameter.schema
    }
    return acc
  }, {})
