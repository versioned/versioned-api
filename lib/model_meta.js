const {find, merge, getIn, keys} = require('lib/util')
...
function titleProperty (model) {
  const propertyNames = keys(properties(model))
  const fallbacks = ['title', 'name', 'id']
  return getIn(model, 'schema.x-meta.titleProperty') || find(fallbacks, name => propertyNames.includes(name))
}
