const {compact, flatten, empty, omit, getIn, merge, json, parseJson, prettyJson} = require('lib/util')
...
function schemaPath (path, schema) {
  if (empty(path) || empty(schema)) return []
  const [key, ...childPath] = path
  let _schemaPath = []
  if (schema.type === 'object') {
    _schemaPath = ['properties', key]
  } else if (schema.type === 'array') {
    _schemaPath = ['items', 'properties', key]
  }
  const childSchema = getIn(schema, _schemaPath)
  return flatten([..._schemaPath, ...schemaPath(childPath, childSchema)])
}
