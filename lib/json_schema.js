  ...
  if (errors) {
    ...
    const formattedErrors = compact(errors.map(error => {
      if (error.keyword === 'additionalProperties' && error.dataPath === '') {
        ...
        const field = error.schemaPath.match(/properties\/([^/]+)/)[1]
        ...
        console.error(`jsonSchema.formatErrors - could not parse error=${prettyJson(error)} for doc=${prettyJson(doc)}`)
        return error
      }
    }))
