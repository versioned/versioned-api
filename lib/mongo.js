...
function mongo (url) {
  ...
  function formatDbError (error) {
    // Don't wrap the error twice
    if (error.status && error.errors) return error
    ...
    if (error.code === 11000) { // DuplicateKey
      ...
      const fieldMatch = error.errmsg.match(/index: ([a-z._]+?)_\d/)
      if (fieldMatch) {
        const message = `Duplicate value for ${fieldMatch[1]} - must be unique`
