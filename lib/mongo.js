...
function mongo (url) {
  ...
  function connect () {
    ...
    return new Promise((resolve, reject) => {
      const options = {connectTimeoutMS: 10000, useUnifiedTopology: true}
  ...
  // }
  async function nextSequence (name, options = {}) {
    const query = {_id: name}
    const sort = [['_id', 'asc']]
    const update = (options.value ? {$set: {seq: parseInt(options.value)}} : {$inc: { seq: 1 }})
    const modifyOptions = {upsert: true, new: true}
    return (await connect()).collection('sequences').findAndModify(query, sort, update, modifyOptions)
