...
function validApiKey (req) {
  const routeHasPublishedParam = getIn(req, 'route.parameters', []).find(p => p.name === 'published' && p.in === 'query')
  return (req.method === 'GET' || req.route.allowApiKey) &&
    req.space &&
    req.queryParams.apiKey === req.space.apiKey &&
    (!routeHasPublishedParam || (req.queryParams.published || req.queryParams.versionToken))
}
