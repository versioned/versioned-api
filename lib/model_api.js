const {filter, keyValues, isArray, rename, pick, evolveAll, isObject, keys, getIn, concat, empty, json, merge, compact} = require('lib/util')
...
function propertyHasDefaultIndex (property) {
  if (getIn(property, 'x-meta.index') !== false &&
        (getIn(property, 'x-meta.relationship') ||
        ['integer', 'number'].includes(property.type) ||
        (property.type === 'string' && getIn(property, 'maxLength') && getIn(property, 'maxLength') <= 1024))) {
    return true
  } else {
    return false
  }
...
function modelIndexes (model) {
  const propertyIndexes = compact(keys(model.schema.properties).map(property => {
    let {index, unique, sequence} = getIn(model, ['schema', 'properties', property, 'x-meta'], {})
    if (sequence) unique = {index: true}
    if (!index) index = propertyHasDefaultIndex(property)
    if (index || getIn(unique, 'index')) {
...
function modelApi (model, mongo, logger = SILENT_LOGGER) {
  ...
  function defaultSort () {
    if (modelMeta.hasProperty(model, 'updatedAt')) {
      return [['updatedAt', -1]]
    } else {
      return undefined
    }
    ...
    return keys(queryParams).reduce((acc, name) => {
      ...
      if (fieldNameMatch) {
        ...
        if (schema) {
          let op = 'eq'
          if (name.endsWith(']')) {
            const OP_PATTERN = /\[(eq|ne|in|exists|regex|lt|gt|lte|gte)\]$/i
          ...
          if (op === 'eq') {
            ...
            acc[fieldName] = {$regex: value, $options: 'i'}
          } else {
            const mongoOp = ('$' + op)
  ...
  function searchQuery (options) {
    const q = getIn(options, 'queryParams.q')
    if (q) {
      const searchConditions = compact(keyValues(getIn(model, 'schema.properties')).map(([key, property]) => {
        if (property.type === 'string') {
          return {[key]: {$regex: q, $options: 'i'}}
        }
      }))
  ...
  async function list (query = {}, options = {}) {
    const queryWithFilter = merge(filteredQuery(query, options), searchQuery(options))
