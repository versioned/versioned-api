...
function modelApi (model) {
  ...
  async function update (id, doc, options = {}) {
    ...
    updatedDoc = compact(await modelCallbacks(model, updatedDoc, 'before_validation', 'update', options))
    ...
    if (errors) return Promise.reject(errors)
    updatedDoc = compact(await modelCallbacks(model, updatedDoc, 'after_validation', 'update', options))
    await createIndexes(model.coll, model.schema)
    return db().collection(model.coll).update(idQuery(id), updatedDoc)
      .then((result) => {
        return modelCallbacks(model, updatedDoc, 'after_save', 'update', options)
      })
      .catch(dbError => {
