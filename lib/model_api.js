...
function modelApi (model, logger = SILENT_LOGGER) {
  ...
  async function list (query = {}, options = {}) {
    ...
    const limit = parseInt(options.limit || LIST_LIMIT)
    const skip = parseInt(options.skip || 0)
    const findOptions = {sort, limit, skip}
    return db().collection(model.coll).find(query, findOptions).toArray()
  }
  ...
  async function update (id, doc, options = {}) {
    ...
    logger.debug(`model_api: ${model.coll}.update id=${id} doc=${json(doc)} changes=${json(changes(existingDoc, updatedDoc))} updatedDoc=${json(updatedDoc)} query=${json(idQuery(id))}`)
    await createIndexes(model.coll, modelIndexes(model))
