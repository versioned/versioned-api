const {pick, evolveAll, evolve, isObject, keys, getIn, concat, empty, json, merge, compact, omit} = require('lib/util')
...
function modelApi (model, mongo, logger = SILENT_LOGGER) {
  ...
  const {db, collStats, formatDbError, createIndexes, isMongoId, makeMongoId} = mongo
  model = modelSpec.generate(model)
  const idProperty = modelMeta.idProperty(model)

  function idQuery (id) {
    if (isMongoId(id)) {
      return {_id: makeMongoId(id)}
    } else {
  ...
  function makeQuery (queryOrId) {
    if (isObject(queryOrId)) {
      return evolve(queryOrId, {_id: makeMongoId})
    } else {
  ...
  async function findOne (queryOrId, options = {}) {
    const query = makeQuery(queryOrId)
    const result = await db().collection(model.coll).findOne(query)
    assertScoped(result, query, options)
    return result
  }
  ...
  async function create (doc, options = {}) {
    ...
    return db().collection(model.coll).insert(dbDoc)
      ...
      .catch(dbError => {
        logger.debug(`model_api: ${model.coll}.create dbError dbError=${json(dbError)}`, dbError)
  ...
  async function update (queryOrId, doc, options = {}) {
    ...
    if (empty(changes(existingDoc, updatedDoc))) return Promise.reject(unchangedError(model, updatedDoc))
    const query = makeQuery(queryOrId)
    logger.debug(`model_api: ${model.coll}.update queryOrId=${json(queryOrId)} doc=${json(doc)} existingDoc=${json(existingDoc)} updatedDoc=${json(updatedDoc)} changes=${json(changes(existingDoc, updatedDoc))} query=${json(query)}`)
    await createIndexes(model.coll, modelIndexes(model))
    return db().collection(model.coll).update(query, updatedDoc)
      .then((result) => {
        if (result.result.nModified === 0) return Promise.reject(dbResultError(model, updatedDoc, result.result, 'database update failed'))
        return modelCallbacks(api, updatedDoc, 'afterSave', 'update', callbackOptions)
      })
      .catch(dbError => {
        logger.debug(`model_api: ${model.coll}.update dbError dbError=${json(dbError)}`, dbError)
        throw formatDbError(dbError)
      })
  }
  ...
  async function _delete (queryOrId, options = {}) {
    ...
    return db().collection(model.coll).remove(query)
      .then((result) => {
        if (result.result.ok !== 1) return Promise.reject(dbResultError(model, doc, result.result, 'database delete failed'))
        return modelCallbacks(api, doc, 'after', 'delete', options)
      })
      .catch(dbError => {
        logger.debug(`model_api: ${model.coll}.delete dbError dbError=${json(dbError)}`, dbError)
        throw formatDbError(dbError)
      })
  }
