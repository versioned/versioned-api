...
const {db, collStats, formatDbError, createIndexes, isMongoId, createId} = require('lib/mongo')
...
function modelApi (model, logger = SILENT_LOGGER) {
  ...
  async function list (query = {}, options = {}) {
    const sort = parseSort(options.order) || defaultSort()
    const limit = options.limit || LIST_LIMIT
    const skip = options.skip || 0
    const findOptions = {sort, limit, skip}
    let findArgs = await modelCallbacks(model, {query, findOptions}, 'before', 'list', options)
    let result = await db().collection(model.coll).find(findArgs.query, findArgs.findOptions).toArray()
    result = await modelCallbacks(model, result, 'after', 'list', options)
    return result
  }

  async function get (id, options = {}) {
    let findArgs = await modelCallbacks(model, {query: idQuery(id), id}, 'before', 'get', options)
    let result = await db().collection(model.coll).findOne(findArgs.query)
    result = await modelCallbacks(model, result, 'after', 'get', options)
    return result
  }
  ...
  async function create (doc, options = {}) {
    let dbDoc = compact(await modelCallbacks(model, doc, 'beforeValidation', 'create', options))
    ...
    dbDoc = compact(await modelCallbacks(model, dbDoc, 'afterValidation', 'create', options))
    ...
    return db().collection(model.coll).insert(dbDoc)
      .then(result => {
        ...
        return modelCallbacks(model, resultDoc, 'afterSave', 'create', options)
      })
      .catch(dbError => {
  ...
  async function update (queryOrId, doc, options = {}) {
    ...
    updatedDoc = compact(await modelCallbacks(model, updatedDoc, 'beforeValidation', 'update', callbackOptions))
    ...
    updatedDoc = compact(await modelCallbacks(model, updatedDoc, 'afterValidation', 'update', callbackOptions))
    ...
    return db().collection(model.coll).update(query, updatedDoc)
      .then((result) => {
        return modelCallbacks(model, updatedDoc, 'afterSave', 'update', callbackOptions)
      })
      .catch(dbError => {
